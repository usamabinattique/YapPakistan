...
public protocol AuthenticationServiceType {
    ...
    func saveProfile<T: Codable>(firstName: String, lastName: String, email: String, companyName: String?, countryCode: String, phone: String, passcode: String, accountType: String, token: String?) -> Observable<T>
...
public class AuthenticationService: AuthBaseService, AuthenticationServiceType {
    ...
    public func authenticate<T: Codable>(username: String, password: String, deviceId: String) -> Observable<T> {
        let params = ["client_id": username, "client_secret": password, "grant_type": "client_credentials", "device_id": deviceId]
        let input: RouterInput<[String:String]> = RouterInput(body: params, query: nil, pathVariables: nil, headers: authorizationProvider.authorizationHeaders)
        let authRoute = AuthRouter.authenticate(input)
        return request(apiClient: apiClient, route: authRoute)
    }
    
    public func saveProfile<T: Codable>(firstName: String, lastName: String, email: String, companyName: String?, countryCode: String, phone: String, passcode: String, accountType: String, token: String?) -> Observable<T> {
        Observable.empty()
    }
