...
@IBDesignable open class RangeSeekSlider: UIControl {
    ...
    open override func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        if let delg = delegate?.didStartTouches {
            delg(self)
        }
    ...
    open override func endTracking(_ touch: UITouch?, with event: UIEvent?) {
        ...
        if let delg = delegate?.didEndTouches {
            delg(self)
        }
    }
    ...
    fileprivate func refresh() {
        ...
        // update the delegate
        if let _ = delegate?.rangeSeekSlider?(self, didChange: selectedMinValue, maxValue: selectedMaxValue), handleTracking != .none {
