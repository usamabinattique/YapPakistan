...
class ChangePhoneNumberViewController: UIViewController {
    ...
    // MARK: - Init
    init(viewModel: ChangePhoneNumberViewModelType) {
    ...
    // MARK: - Views
    private lazy var headingLabel: UILabel = UILabelFactory.createUILabel(with: .primaryDark, textStyle: .title2, alignment: .center)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        title = SessionManager.current.currentAccountType == .b2cAccount ? nil : "Edit phone number"
        setup()
        bind()
    }
