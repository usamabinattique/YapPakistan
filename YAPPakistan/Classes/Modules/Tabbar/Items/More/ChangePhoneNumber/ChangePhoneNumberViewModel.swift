...
private extension ChangePhoneNumberViewModel {
    ...
    func attributed(text: String) -> NSAttributedString {
        ...
        attributed.addAttributes([NSAttributedString.Key.foregroundColor: UIColor.appColor(ofType: .primaryDark).withAlphaComponent(0.5)], range: NSRange(location: 0, length: length == 0 ? text.count : length))
        if length > 0 {
            attributed.addAttributes([NSAttributedString.Key.foregroundColor: UIColor.appColor(ofType: .primaryDark)], range: NSRange(location: length, length: text.count - length))
        }
        return attributed
    }
