...
class QRPaymentViewController: UIViewController {
    ...
    private lazy var currency = UIFactory.makeLabel(font:.title1)
    ...
    private lazy var feeLabel = UIFactory.makeLabel(font: .micro, alignment: .center, numberOfLines: 0, lineBreakMode: .byWordWrapping)

    private lazy var balance = UIFactory.makeLabel(font:.micro,alignment: .center,numberOfLines: 0,lineBreakMode: .byWordWrapping)
    ...
    private lazy var confirmButton = AppRoundedButtonFactory.createAppRoundedButton(title: "Pay")

    // MARK: Properties
    private var themeService: ThemeService<AppTheme>!
    private var viewModel: QRPaymentViewModelType!
    ...
    // MARK: Initialization
    init(themeService: ThemeService<AppTheme>, viewModel: QRPaymentViewModelType) {
    ...
    func setupTheme() {
        themeService.rx
         ...
         //  .bind({ UIColor( $0.greyLightSecondary)}, to: [amountView.rx.backgroundColor])
            .bind({ UIColor($0.greyDark) }, to: [feeLabel.rx.textColor,phoneNumber.rx.textColor, balance.rx.textColor])
        //    .bind({ UIColor($0.primaryExtraLight) }, to: [balance.rx.textColor])
            .bind({ UIColor($0.primaryDark)}, to: [backButton.rx.tintColor, userName.rx.textColor])
...
private extension QRPaymentViewController {
    func setupViews() {
...
extension QRPaymentViewController: UITextFieldDelegate {
...
fileprivate extension QRPaymentViewController {
