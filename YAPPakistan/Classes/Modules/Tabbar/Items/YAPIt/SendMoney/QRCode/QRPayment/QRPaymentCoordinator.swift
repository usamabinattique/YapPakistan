...
public class QRPaymentCoordinator: Coordinator<ResultType<Void>> {
...
private extension QRPaymentCoordinator {
   /* func otp(_ contact: YAPContact, _ amount: Double, result: AnyObserver<Void>) {
    ...
    func tranferSuccess(_ contact: YAPContact, _ transaction: Y2YTransactionResponse) {
        let viewModel = QRPaymentSuccessViewModel(contact, transaction)
        let viewController = QRPaymentSuccessViewController(theme: container.themeService, viewModel: viewModel)
        if self.shouldPresent {
