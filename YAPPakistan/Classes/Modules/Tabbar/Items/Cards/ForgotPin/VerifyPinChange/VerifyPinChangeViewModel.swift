//
//  VerifyPinChangeViewModel.swift
//  YAPPakistan
//
...
class VerifyPinChangeViewModel: VerifyMobileOTPViewModel {
    private let username: String
    private let passcode: String
    var timerDisposable: Disposable?

    init(action: OTPAction = .forgotPIN,
         heading: String? = nil,
         subheading: String,
         image: UIImage? = nil,
         badge: UIImage? = nil,
    ...
    override func generateOneTimePasscode(mobileNo: String) {
        let generateOTPRequest = generateOTPSubject.skip(1)
            ...
            .flatMap { [unowned self] _ -> Observable<Event<String?>> in
                return self.repository.generate(action: OTPAction.forgotPIN.rawValue)
            }
    ...
    override func verifyOneTimePasscode(mobileNo: String, passcode: String) {
        let verifyRequest = sendSubject.withLatestFrom(textSubject.unwrap())
            ...
            .flatMap { [unowned self] otp -> Observable<Event<String?>> in
                self.repository.verifyOTP(action: OTPAction.forgotPIN.rawValue, otp: otp)
            }
            .do(onNext: { _ in YAPProgressHud.hideProgressHud() })
            .share()
