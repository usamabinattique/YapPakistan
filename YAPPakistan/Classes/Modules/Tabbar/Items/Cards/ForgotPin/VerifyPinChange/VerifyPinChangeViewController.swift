//
//  VerifyPinChangeViewController.swift
//  YAPPakistan
//
...
public class VerifyPinChangeViewController: UIViewController {
    ...
    private lazy var headingLabel = UIFactory.makeLabel(font: .title2,
                                                        alignment: .center,
                                                        adjustFontSize: true)
    private lazy var subHeadingLabel = UIFactory.makeLabel(font: .regular,
                                                           alignment: .center,
                                                           numberOfLines: 2,
    ...
    var viewModel: VerifyPinChangeViewModelType!

    init(themeService: ThemeService<AppTheme>, viewModel: VerifyPinChangeViewModelType) {
...
extension VerifyPinChangeViewController {
    func setupViews() {
    ...
    func setupConstraints() {
        ...
        sendButtonBottomConstraint = view.safeAreaLayoutGuide.bottomAnchor
            .constraint(equalTo: sendButton.bottomAnchor, constant: 12)
...
private extension VerifyPinChangeViewController {
    func bindViews() {
...
//            if flag == true {
//                self?.headingLabel.topAnchor
//                    .constraint(equalTo: (self?.view.safeAreaLayoutGuide.topAnchor)!, constant: 52)
//                    .isActive = true
//            } else {
//                self?.headingLabel.topAnchor
//                    .constraint(equalTo: (self?.logoImageView.bottomAnchor)!, constant: 30)
//                    .isActive = true
//            }
...
//            .disposed(by: disposeBag)
        ...
        sendButton.rx.tap.do(onNext: { [weak self] in self?.view.endEditing(true) })
            .bind(to: viewModel.inputs.sendObserver)
            .disposed(by: disposeBag)
//        resendButton.rx.tap.do(onNext: { [weak self] in self?.view.endEditing(true) })
//            .bind(to: viewModel.inputs.generateOTPObserver)
//            .disposed(by: disposeBag)
//
...
extension VerifyPinChangeViewController: UITextFieldDelegate {
