...
class ReportCardViewModel : ReportCardViewModelInputs, ReportCardViewModelOutputs, ReportCardViewModelType {
    ...
    var cardPlan: Observable<String> {
        return paymentCardSubject.map {
            $0.cardType == .debit ? "common_disply_text_card_name_primary_silver".localized:
                $0.physical ? "common_display_text_physical_card".localized:
                "common_display_text_virtual_card".localized }
    }
    ...
    var blockConfirmationAlert: Observable<UIAlertControlTemplate> {
        return blockButtonTap.map { _ in (
            "screen_report_card_display_text_block_alert_title".localized,
            "screen_report_card_display_text_block_alert_message".localized,
            "common_button_confirm".localized,  "common_button_cancel".localized
        ) }
    }
