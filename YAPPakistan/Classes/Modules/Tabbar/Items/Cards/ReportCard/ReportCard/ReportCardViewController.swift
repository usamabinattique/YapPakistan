...
fileprivate extension ReportCardViewController {
    ...
    func bindAlertControl() {
        viewModel.outputs.blockConfirmationAlert.subscribe(onNext: { [weak self] alertControlTemplate in
            self?.showAlert(title: alertControlTemplate.title,
                            message: alertControlTemplate.message,
                            defaultButtonTitle: alertControlTemplate.defaultButtonTitle,
                            secondayButtonTitle: alertControlTemplate.secondaryButtonTitle,
    ...
    func setupTheme() {
        themeService.rx
            ...
            .bind({ UIColor($0.greyDark) }, to: [blockReportButton.rx.disabledBackgroundColor])
