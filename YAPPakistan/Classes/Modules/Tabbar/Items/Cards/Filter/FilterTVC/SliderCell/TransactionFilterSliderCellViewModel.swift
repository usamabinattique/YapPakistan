...
class TransactionFilterSliderCellViewModel: TransactionFilterSliderCellViewModelType,
    ...
    private let progressSubject = BehaviorSubject<CGFloat>(value: 0)
    ...
    // MARK: - Inputs
    var progressObserver: AnyObserver<CGFloat> { return progressSubject.asObserver() }
    ...
    init(_ range: ClosedRange<Double>, _ selectedRange: ClosedRange<Double> = 0...0) {
        ...
        selectedRangeSubject.map { String.init(
            format: "%@ — %@",
            NumberFormatter.formateAmount(range.lowerBound, fractionDigits: 0),
